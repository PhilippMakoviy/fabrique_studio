...
def createPoll(pollData):
    res = requests.post(API_URL + '/admin/polls', auth=basicAuth, data=pollData['poll'])
...
class TestUser:
    ...
    def test_submit_text_poll(self, textPollId):
        ...
        res = requests.post(API_URL + '/polls/%d' % textPollId, data=submitData)
        assert res.status_code == 200
    ...
    def test_submit_choice_poll(self, choicePollId):
        ...
        res = requests.post(API_URL + '/polls/%d' % choicePollId, data=submitData)
        assert res.status_code == 200
