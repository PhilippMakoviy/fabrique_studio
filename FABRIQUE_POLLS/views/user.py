...
from ..models import Poll, Finished_poll , Answer
from ..serializers import PollSerializer, QuestionSerializer, UOptionSerializer, Finished_poll_Serializer


class Polls(APIView):
    def get(self, request):
        today = date.today()
        pollSet = Poll.objects.filter(start_date__lte=today, end_date__gt=today)
...
class PollById(APIView):
    def get(self, request, id):
        try:
            ...
            if poll.start_date > today or poll.end_date < today:
            ...
            for question in poll.question_set.all():
                ...
                if question.OptionType:
                    questionDict['options'] = UOptionSerializer(question.option_set.all(), many=True).data
    ...
    def post(self, request, id):
        try:
            ...
            if Finished_poll.objects.filter(userId=userId, poll=poll).count() > 0:
            ...
            def makeAnswer(question):
                ...
                if question.type == 'CHOICE':
                    ...
                    if foundOption:
                        answer.answer_text = foundOption.text
                    else:
                ...
                if question.type == 'MULTIPLE_CHOICE':
                    ...
                    for index in answerData:
                        ...
                        if foundOption:
                            ...
                            raise invalidIndexException
                    answer.answer_text = json.dumps(resultList)
            ...
            submis = Finished_poll(userId=userId, poll=poll)
            submis.save()
            for answer in answerList:
                answer.finished_poll = submis
                answer.save()
...
class PollsByUser(APIView):
    def get(self, request, id):
        try:
            result = []
            for finished_poll in Finished_poll.objects.filter(userId=id).order_by('submit_time'):
                Finished_pollDict = Finished_poll_Serializer(finished_poll).data
                Finished_pollDict['poll_id'] = finished_poll_id
                Finished_pollDict['answers'] = []
                for answer in finished_poll.answer_set.all():
                    answer_text = answer.answer_text
                    if answer.question_type == 'MULTIPLE_CHOICE':
                        answer_text = json.loads(answer_text)

                    Finished_pollDict['answers'].append({
                        'question': {
                ...
                result.append(Finished_pollDict)
